...
void TyreThermalModel::sethotTemperature(float newbcoretemp, float newpatch)
{
	coreTemp = newbcoretemp;
	for (auto& patch : patches)
	{
		patch.T = newpatch;
	}
...
{
	setTemperature(car->sim->ambientTemperature);  //可以直接更新为ac提供的核心温度
	phase = 0;
}
