...
{
	clutchOpenState = true; //（离合开），不考虑，代码内会进行计算
	rootVelocity = 0; //不考虑选档，车辆正常行驶，rootVelocity=engine.velocity
	engine.velocity = 0; //有，根据engineRPM进行换算
	outShaftL.velocity = 0; //有，根据WheelAngularSpeed来获取，这是驱动轮的速度
	outShaftR.velocity = 0; //有，根据WheelAngularSpeed来获取，这是驱动轮的速度
	outShaftLF.velocity = 0; //ae86是后驱车，所以不考虑这个
	outShaftRF.velocity = 0; //ae86是后驱车，不考虑这个
	drive.velocity = 0; //engine.velocity=rootVelocity=drive.velocity×ig×ifinal其中𝑖𝑔 是当前档位齿比，𝑖𝑓𝑖𝑛𝑎𝑙 是主减速比。或drive.velocity=(outShaftL.velocity+outShaftR.velocity)/2
	gearRequest.request = GearChangeRequest::eNoGearRequest;//换挡过程中不考虑
	validShiftRPMWindow = orgRpmWindow; //换挡过程中不考虑
	engineModel->reset();
}
...
void Drivetrain::step2WD(float dt)
{
    ...
    switch (currentGear) {
        case 2: ratiotime = 1; break;
        case 3: ratiotime = 1; break;
        case 4: ratiotime = 1; break;
        case 5: ratiotime = 1; break;
        default: ratiotime = 1; break;
    }

	switch (currentGear) {
        case 2: ratioreal = 1; break;
        case 3: ratioreal = 1; break;
        case 4: ratioreal = 1; break;
        case 5: ratioreal = 1; break;
        default: ratioreal = 1; break;   //数值过大则是ac>sim，过小是sim>ac
    }

	// switch (currentGear) {
    //     case 2: ratiotime = 0.90; break;
    //     case 3: ratiotime = 0.89; break;
    //     case 4: ratiotime = 0.88; break;
    //     case 5: ratiotime = 0.81; break;
    //     default: ratiotime = 1; break;
    // }

	// switch (currentGear) {
    //     case 2: ratioreal = 0.99; break;
    //     case 3: ratioreal = 0.96; break;
    //     case 4: ratioreal = 0.94; break;
    //     case 5: ratioreal = 0.82; break;
    //     default: ratioreal = 1; break;   //数值过大则是ac>sim，过小是sim>ac
    // }

	ratio = finalRatio * ratiotime * curGear.ratio;
	...
	if (locClutch < 1.0f)
	{
		clutchOpenState = true;   //如果为true就说明离合器打开了，离合踩下了
	}
	else if (engine.velocity != 0.0)
	{
		clutchOpenState = (fabs(rootVelocity / engine.velocity - 1.0) >= 0.1); //驱动与发动机差10%说明打开了，否则就是闭合了，
	}
	else
	{
		clutchOpenState = (rootVelocity != 0.0); //看驱动的状态
	}
	...
	if (!clutchOpenState)
	{
		double fDeltaRootV = (engineModel->status.outTorque*enginetorque / fNewEngineInertia) * dt;
		...
		if (ratio == 0.0)
		{
			...
			drive.velocity += fdelta*fDeltaDriveV;
		}
		else
		{
			accelerateDrivetrainBlock(fDeltaRootV / ratio, true);

			fDeltaDriveV = fdelta*(tyreRight->status.feedbackTorque + tyreLeft->status.feedbackTorque) / fInertiaFromWheels * dt; //改动
	...
	else
	{
		...
		if (ratio != 0.0)
		{
			engine.velocity += (fClutchTorq + engineModel->status.outTorque*enginetorque ) / fEngineInertia * dt;
		...
		{
			double fNewEngineVelocity = engine.velocity + engineModel->status.outTorque*enginetorque  / fEngineInertia * dt;
	...
	else if (diffType == DifferentialType::LSD)
	{
		...
		if (fClutchTorq != 0.0)
			...
			fOutClutchTorq = locClutch * engineModel->status.outTorque*enginetorque ;
	...
	if (tyreLeft->status.isLocked && tyreRight->status.isLocked)
	{
		...
		if (fabs(ratio * engineModel->status.outTorque*enginetorque ) <= (fTorqL + fTorqR))
		{
	...
	if (ratio == 0.0)
	{
		totalTorque = fabs(engineModel->status.outTorque*enginetorque * locClutch);
	}
	else
	{
		totalTorque = fabs((fabs(ratio) * (engineModel->status.outTorque*enginetorque * locClutch)) - (tyreLeft->status.feedbackTorque + tyreRight->status.feedbackTorque));
	}

	float fGearTorque = (float)(locClutch * engineModel->status.outTorque*enginetorque * curGear.ratio);  
...
void Drivetrain::reallignSpeeds(float dt, double ratioreal) //改动
{
	double fRatio = ratio/(ratioreal/2+0.5);  //不除则是ac>sim，除则是sim>ac,分母越小，sim越大，
	if (fRatio != 0.0)
	{
