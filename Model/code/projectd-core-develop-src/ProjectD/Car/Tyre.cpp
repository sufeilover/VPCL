...
void Tyre::reset()
{
	...
	status.slipAngleRAD = 0; //有
	status.slipRatio = 0; //有
	status.angularVelocity = 0; //有
	status.Fy = 0; //这部分没有暴露，但是在step内会被置0，因此热启动不需要管
	status.Fx = 0; //这部分没有暴露，但是在step内会被置0，因此热启动不需要管
	status.Mz = 0; //有
	status.isLocked = 1; //这部分由于ac没有暴露，可以在python内根据ac提供的数据进行计算，标识当前车轮是否被完全锁死，但是我们的模型应该不考虑这个
	status.inflation = 1; //充气比，一直是1，没有相关的计算，热启动不管
	status.flatSpot = 0; //这部分由于ac没有暴露，可以在python内根据ac提供的数据进行计算，制动过程中，若车轮长时间抱死（isLocked=1 且存在滑移），模型会累积一个 flatSpot 值，轮胎抱死之后的影响
	//在我们的模型中不考虑这种情况，我们的模型是车辆一直正常形式，不正常就不预测，所以不考虑这个
	status.lastGrain = (float)status.grain;   //目前由于tyreConsumptionRate一直为0，这部分也一直为0，这部分不执行，轮胎无磨损
	status.lastBlister = (float)status.blister;//目前由于tyreConsumptionRate一直为0，这部分也一直为0，这部分不执行，轮胎无磨损
	status.grain = 0;//目前由于tyreConsumptionRate一直为0，这部分也一直为0，这部分不执行，轮胎无磨损
	status.blister = 0;//目前由于tyreConsumptionRate一直为0，这部分也一直为0，这部分不执行，轮胎无磨损

	rSlidingVelocityX = 0; //这部分没有暴露，但是在step内会被置0，因此热启动不需要管
	rSlidingVelocityY = 0; //这部分没有暴露，但是在step内会被置0，因此热启动不需要管

	if (status.virtualKM > 0.001f) //目前由于tyreConsumptionRate一直为0，这部分也一直为0
		thermalModel->getIMO(status.lastTempIMO); 
	
	status.dirtyLevel = 0; //有
	status.feedbackTorque = 0; //这部分没有暴露，但是在step内会被置0，因此热启动不需要管
	status.virtualKM = 0; //目前由于tyreConsumptionRate一直为0，这部分也一直为0
	absOverride = 1; //abs一直是1，相当于不启用
	tyreBlanketsOn = true; //是否暖胎

	thermalModel->reset();
	
	// ====== 【新增】重置时立刻用最新 hub 位姿做一次地面射线，并回写 Tyre 缓存 ======
    // 1) 直接从 hub 的世界矩阵取位置（这是最新的瞬移后姿态）
    if (hub && (rayCaster || rayCastProvider))
    {
        const mat44f mxHub = hub->getHubWorldMatrix();
        const vec3f hubPos(mxHub.M41, mxHub.M42, mxHub.M43);

        // 2) 与 step() 规则一致：轮心上方 2m，向下
        const vec3f vRayPos(hubPos.x, hubPos.y + 2.0f, hubPos.z);
        const vec3f vRayDir(0.0f, -1.0f, 0.0f);

        bool bHasContact = false;
        vec3f vHitPos(0,0,0), vHitNorm(0,1,0);
        Surface* pSurface = nullptr;

        // 3) 先用“长射线”确保命中（防止 3m 长度不够）——优先走 provider
        if (rayCastProvider)
        {
            TrackRayCastHit thit{};
            rayCastProvider->rayCast(vRayPos, vRayDir, /*length=*/10.0f, thit);
            if (thit.hasContact)
            {
                bHasContact = true;
                vHitPos     = thit.pos;
                vHitNorm    = thit.normal;
                pSurface    = (Surface*) thit.surface; // provider 直接给 Surface*
            }
        }

        // 4) 如仍未命中，再用本轮的 RayCaster（长度 3m）
        if (!bHasContact && rayCaster)
        {
            ...
            if (hit.hasContact)
            {
                ...
                vHitPos     = hit.pos;
                vHitNorm    = hit.normal;
                if (auto* obj = (ICollisionObject*)hit.collisionObject)
                    pSurface = (Surface*) obj->getUserPointer();
            }
        }

        // 5) 把这一次的结果“定格”到轮胎缓存，避免残留瞬移前的接触点
        if (bHasContact)
        {
            surfaceDef              = pSurface;
        ...
        {
            surfaceDef              = nullptr;
            unmodifiedContactPoint  = vec3f(0,0,0);
            contactPoint            = vec3f(0,0,0);
            contactNormal           = vec3f(0,1,0);
        }

        // 6) 顺手把 RayCaster 的起点/方向也同步成与 step() 一致的配置
        if (rayCaster)
            (void)rayCaster->rayCast(vRayPos, vRayDir); // 内部会 dGeomRaySet(pos,dir)
    }
}
