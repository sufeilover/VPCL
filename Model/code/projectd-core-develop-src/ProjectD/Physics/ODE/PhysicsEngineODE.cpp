...
// RayCasterODE.cpp
RayParams RayCasterODE::getState() const {
    RayParams p{};
    dVector3 o, d;
    ODE_CALL(dGeomRayGet)(id, &o[0], &d[0]);      // 取起点与方向
    p.pos = vec3f(o[0], o[1], o[2]);
    p.dir = vec3f(d[0], d[1], d[2]);
    p.length       = ODE_CALL(dGeomRayGetLength)(id);
    p.firstContact = ODE_CALL(dGeomRayGetFirstContact)(id);
    p.backfaceCull = ODE_CALL(dGeomRayGetBackfaceCull)(id);
    p.closestHit   = ODE_CALL(dGeomRayGetClosestHit)(id);
    return p;
}

void RayCasterODE::setState(const RayParams& p) {
    ODE_CALL(dGeomRaySet)(id, ODE_V3(p.pos), ODE_V3(p.dir));
    ODE_CALL(dGeomRaySetLength)(id, p.length);
    ODE_CALL(dGeomRaySetFirstContact)(id, p.firstContact);
    ODE_CALL(dGeomRaySetBackfaceCull)(id, p.backfaceCull);
    ODE_CALL(dGeomRaySetClosestHit)(id, p.closestHit);
}

void RayCasterODE::reset(float length) {
    // 一个“软重置”：回到默认参数（原点向 -Y，长度按需）
    ODE_CALL(dGeomRaySet)(id, 0.f, 0.f, 0.f,  0.f,-1.f, 0.f);
    ODE_CALL(dGeomRaySetLength)(id, length);
    ODE_CALL(dGeomRaySetFirstContact)(id, 1);
    ODE_CALL(dGeomRaySetBackfaceCull)(id, 1);
    ODE_CALL(dGeomRaySetClosestHit)(id, 0);
}
